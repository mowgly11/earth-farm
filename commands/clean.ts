...
export async function execute(interaction: CommandInteraction) {
    ...
    if (!userProfile.farm.occupied_animal_slots.length) {
        return await interaction.editReply({ content: "You don't have any animals' area to clean!" });
    }
    ...
    if (lastCleaned + cooldown > now) {
        ...
        return await interaction.editReply({ content: `You need to wait **${timeLeft}** minutes before cleaning again!` });
    }
    ...
    if (timeLeft <= 0) {
        return await interaction.editReply({ content: `The animal in slot **${slotNumber}** is ready to harvest! No need to clean its area.` });
    }
    ...
    return await interaction.editReply({ content: `Successfully cleaned area for animal in slot **${slotNumber}**! Production speed increased by **${boost}%** (Total boost: **${animalSlot.total_boost}%**)` });
}
