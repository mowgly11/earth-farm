...
export async function execute(interaction: CommandInteraction) {
    ...
    if (!userProfile) {
        ...
        if (!dbProfile) return await interaction.editReply({ content: "please use `/farmer` before trying to claim your daily reward." });
    ...
    if (timeLeft > 0) return await interaction.editReply({ content: `You still have to wait **${formatTimestamp(timeLeft)}** to claim your next daily reward.` });
    ...
    return await interaction.editReply({ content: `${userProfile.username} has earned a total of **${reward}** 🪙` });
}
