...
export async function execute(interaction: CommandInteraction) {
  ...
  if (!userProfile) {
    ...
    if (!dbProfile) return await interaction.editReply({ content: `**${user.username}**'s gold stash wasn't found. ${user.id === interaction.user.id ? "please use `/farmer` before trying to check your gold balance." : ""}` });
  ...
  return await interaction.editReply({ content: `**${userProfile.username}** has a total of **${userProfile?.gold}** 🪙` });
}